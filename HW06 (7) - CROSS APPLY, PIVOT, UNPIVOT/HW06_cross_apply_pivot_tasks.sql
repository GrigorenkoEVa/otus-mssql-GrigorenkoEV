...
FROM (
select 
		FORMAT(DATEFROMPARTS(year(ord.OrderDate),month(ord.OrderDate),'01') , 'dd.MM.yyyy') as InvoiceMonth,
		replace (replace (CustomerName,'Tailspin Toys (',''),')','') cn,
		count (ord.OrderID) as cnt
...
PIVOT
(   count(cnt)
    FOR cn IN ("Gasport, NY", "Jessie, ND", "Medicine Lodge, KS", "Peeples Valley, AZ", "Sylvanite, MT")
) AS pvt
